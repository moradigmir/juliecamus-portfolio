...
export const useMediaIndex = (): UseMediaIndexReturn => {
  ...
  // Background preemptive healer - runs after initial load, silently fixes broken URLs
  useEffect(() => {
    if (mediaItems.length === 0 || isLoading) return;
    
    const healAllItems = async () => {
      console.log(`🩹 [PREEMPTIVE-HEAL] Starting background heal for ${mediaItems.length} items`);
      
      const CONCURRENCY = 6;
      const queue = [...mediaItems];
      let healedCount = 0;
      
      async function healOne(item: MediaItem) {
        try {
          // Extract directory from preview URL
          const u = new URL(toProxyStrict(item.previewUrl));
          const path = u.searchParams.get('path') || '';
          const dir = path.includes('/') ? path.slice(0, path.lastIndexOf('/') + 1) : '/';
          
          // Try to find actual preview in folder
          const correctPreview = await findPreviewForFolder(dir);
          
          if (correctPreview && correctPreview !== item.previewUrl) {
            const lower = correctPreview.toLowerCase();
            const newType: MediaType = /\.(mp4|mov|webm|m4v)(\?|$)/i.test(lower) ? 'video' : 'image';
            
            const healedItem: MediaItem = {
              ...item,
              previewUrl: correctPreview,
              previewType: newType,
              fullUrl: correctPreview,
              fullType: newType,
            };
            
            setMediaItems(prev => 
              prev.map(it => it.folder === item.folder ? healedItem : it)
            );
            
            healedCount++;
            console.log(`🩹 [PREEMPTIVE-HEAL] Healed ${item.folder}: ${correctPreview}`);
            diag('HEAL', 'preemptive_heal', { folder: item.folder, url: correctPreview, type: newType });
          }
        } catch (e) {
          // Silently skip folders that fail
        }
      }
      ...
      if (healedCount > 0) {
        console.log(`✅ [PREEMPTIVE-HEAL] Healed ${healedCount} items`);
      } else {
        console.log(`✅ [PREEMPTIVE-HEAL] All items OK, no healing needed`);
      }
    };
    
    // Run healer after 2s delay to not block initial render
    const timer = setTimeout(healAllItems, 2000);
